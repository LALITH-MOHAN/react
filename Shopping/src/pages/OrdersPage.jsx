...
function OrdersPage() {
  ...
  const { user } = useAuth();
  const [visibleCount, setVisibleCount] = useState(3); //initially 3 orders will be displayed

  const userOrders = Array.isArray(orders) ? orders .filter(order => order?.userId === user?.id) .sort((a, b) => new Date(b.date) - new Date(a.date))  : [];
        ...
        {userOrders.length === 0 ? (
          ...
          (<div className="orders-list">
            {userOrders.slice(0, visibleCount).map(order => {
              ...
              const orderDate = order.date ? new Date(order.date).toLocaleString() : 'Date not available';
